...
//Bien doi tinh tien
void myTranslate(int tx, int ty, int tz)
{
    Matrix4x4 m;
    matrixSetIdentity(m);
    m[0][3] = tx;
...
void display(void)
{
    ...
    {
    case 1:
        myTranslate(TransDistX, TransDistY, TransDistZ);
        break;
    case 2:
    ...
    case 3:
        {
        	...
        	y = (y2 - y11) / MOD;
        	z = (z2 - z1) / MOD;
        	myTranslate(-x1, -y11, -z1);
        	float ThetaDash;
        	ThetaDash = 1260 * atan(y / z) / 22;
        	myRotateX(ThetaDash);
        	myRotateY(1260 * asin(-x) / 22);
        	myRotateZ(Theta);
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("Nhom 26 - Duong Dinh - Khuong Duy - Duc Huynh");
    init();
